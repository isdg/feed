...
feed: jyn
guid: https://jyn.dev/complected-and-orthogonal-persistence/
---

# complected and orthogonal persistence

> ***Everything Not Saved Will Be Lost***—Ancient Nintendo Proverb

## persistence is hard [Anchor link for: persistence-is-hard](https://jyn.dev/complected-and-orthogonal-persistence/\#persistence-is-hard)
...
## languages with persistence [Anchor link for: languages-with-persistence](https://jyn.dev/complected-and-orthogonal-persistence/\#languages-with-persistence)

this [1983 paper](https://archive.cs.st-andrews.ac.uk/papers/download/ABC+83b.pdf) asks: why are we spending so much time rewriting this in applications, instead of doing it once in the runtime? it then introduces a language called "PS-algol", which supports exactly this through a library. note that arbitrary data types in the language are supported without the need for writing user code.

it turns out that this idea is already being used in production. not in that form—people don’t use Algol anymore—but the idea is the same. M (better known as [MUMPS](https://en.wikipedia.org/wiki/MUMPS)), [Bank Python](https://calpaterson.com/bank-python.html), and the [IBM i](https://www.devever.net/~hl/f/as400guide.pdf) [1](https://jyn.dev/complected-and-orthogonal-persistence/#fn-4) are still used in healthcare, financial, and insurance systems, and they work exactly like this[2](https://jyn.dev/complected-and-orthogonal-persistence/#fn-5). here is a snippet of M that persists some data to a database:
...
and finally some COBOL: [3](https://jyn.dev/complected-and-orthogonal-persistence/#fn-3)
...
if you don't require the runtime to support all datatypes, there are frameworks for doing this as a library. [protobuf](https://protobuf.dev/) and [flatbuffer](https://flatbuffers.dev/) both autogenerate the code for a restricted set of data types, so that you only have to write the code invoking it.

## orthogonal persistence [Anchor link for: orthogonal-persistence](https://jyn.dev/complected-and-orthogonal-persistence/\#orthogonal-persistence)
...
## how far can we take this? [Anchor link for: how-far-can-we-take-this](https://jyn.dev/complected-and-orthogonal-persistence/\#how-far-can-we-take-this)
...
- a filesystem that supports atomic accesses, snapshots, and transaction restarts, such as [ZFS](https://en.wikipedia.org/wiki/ZFS#Snapshots_and_clones).
- a runtime that supports detailed tracking and replay of syscalls, such as [rr](https://rr-project.org/). this works by intercepting syscalls with [ptrace()](https://man7.org/linux/man-pages/man2/ptrace.2.html), among other mechanisms, and does not require any modifications to the program executable or source code.
- a sandbox that prevents talking to processes that weren’t running when the target process was spawned (unless those processes are also in the sandbox and tracked with this mechanism), such as [bubblewrap](https://github.com/containers/bubblewrap).

effectively, we are turning syscalls into [capabilities](http://habitatchronicles.com/2017/05/what-are-capabilities/), where the capabilities we give out are “exactly the syscalls the process made last time it spawned”.

note how this is possible to do today, with existing technology and kernel APIs! this doesn’t require building an OS from scratch, nor rewriting all code to be in a language with tracked effects or a capability system. instead, by working at the syscall interface[4](https://jyn.dev/complected-and-orthogonal-persistence/#fn-6) between the program and the kernel, we can build a highly general system that applies to all the programs you already use.

## “but why?” [Anchor link for: but-why](https://jyn.dev/complected-and-orthogonal-persistence/\#but-why)
...
### needs tracking between processes [Anchor link for: needs-tracking-between-processes](https://jyn.dev/complected-and-orthogonal-persistence/\#needs-tracking-between-processes)
...
### needs tracking within a process [Anchor link for: needs-tracking-within-a-process](https://jyn.dev/complected-and-orthogonal-persistence/\#needs-tracking-within-a-process)
...
### needs source code changes [Anchor link for: needs-source-code-changes](https://jyn.dev/complected-and-orthogonal-persistence/\#needs-source-code-changes)
...
## isn’t this horribly slow? [Anchor link for: isn-t-this-horribly-slow](https://jyn.dev/complected-and-orthogonal-persistence/\#isn-t-this-horribly-slow)

not inherently. “turning your file system into a database“ is only as slow as submitting the query is[5](https://jyn.dev/complected-and-orthogonal-persistence/#fn-1)—and that can be quite fast when the database runs locally instead of over the network; see [sled](https://github.com/spacejam/sled?tab=readme-ov-file#performance) for an example of such a DB that has had performance tuning. rr boasts [less than a 20% slowdown](https://rr-project.org/#:~:text=slowdown). bubblewrap uses the kernel’s native namespacing and to my knowledge imposes no overhead.
...
## does this work across versions of my program? [Anchor link for: does-this-work-across-versions-of-my-program](https://jyn.dev/complected-and-orthogonal-persistence/\#does-this-work-across-versions-of-my-program)

kinda. there are two possible ways to implement intra-process persistence (i.e. "everything other than disk writes").

1. take a snapshot of the memory, registers, and kernel state (e.g. file descriptors). this is how [telefork](https://thume.ca/2020/04/18/telefork-forking-a-process-onto-a-different-computer/) works. this only works with a single version of the executable; any change, even LTO without changing source code, will break it.
...
1 is cheap if you don't have much memory usage compared to CPU time. 2 is cheap if you don't have much CPU time compared to memory usage. only 2 allows you to modify the binary between saving and restoring. it's possible to do both for the same process, just expensive.

## summary [Anchor link for: summary](https://jyn.dev/complected-and-orthogonal-persistence/\#summary)
...
## bibliography [Anchor link for: bibliography](https://jyn.dev/complected-and-orthogonal-persistence/\#bibliography)
...
1. the IBM i will be coming up many times in this series, particularly block terminals and the object capability model. i will glaze over parts of the system that cannot be intercepted at a syscall boundary; but that said i want to point out that [IBM POWER extensions](https://www.devever.net/~hl/f/as400guide.pdf#page=13) and [TIMI](https://en.wikipedia.org/wiki/IBM_i#Technology_Independent_Machine_Interface_(TIMI)) correspond roughly to the modern ideas of the [CHERI](https://www.cl.cam.ac.uk/research/security/ctsrd/) and [WASM](https://webassembly.org/) projects. [↩](https://jyn.dev/complected-and-orthogonal-persistence/#fr-4-1)

2. jyn, you might ask, why are all these systems so old! why legacy systems? isn't there any new code with these ideas? and the answer is no. [systems research is irrelevant](https://doc.cat-v.org/bell_labs/utah2000/utah2000.html) and its new ideas, such as they are, do not make it into industry. [↩](https://jyn.dev/complected-and-orthogonal-persistence/#fr-5-1)

3. credit [@yvanscher](https://medium.com/@yvanscher/7-cobol-examples-with-explanations-ae1784b4d576) [↩](https://jyn.dev/complected-and-orthogonal-persistence/#fr-3-1)

4. on all OSes i know other than Linux, the syscall ABI is not stable and programs are expected to use libc bindings. you can do something similar by using LD\_PRELOAD to intercept libc calls. [↩](https://jyn.dev/complected-and-orthogonal-persistence/#fr-6-1)

5. one might think that you have to flush each write to disk before returning from write() in order to preserve ACID semantics. not so; this is exactly the problem that a [write-ahead-log](https://www.postgresql.org/docs/current/wal-intro.html#WAL-INTRO) fixes. [↩](https://jyn.dev/complected-and-orthogonal-persistence/#fr-1-1)
