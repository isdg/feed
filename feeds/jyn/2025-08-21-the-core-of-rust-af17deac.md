...
feed: jyn
guid: https://jyn.dev/the-core-of-rust/
---
...
## Vision [Anchor link for: vision](https://jyn.dev/the-core-of-rust/\#vision)
...
## Learning Rust requires learning many things at once [Anchor link for: learning-rust-requires-learning-many-things-at-once](https://jyn.dev/the-core-of-rust/\#learning-rust-requires-learning-many-things-at-once)
...
Let’s look at a Rust program that does something non-trivial:[1](https://jyn.dev/the-core-of-rust/#fn-1)
...
- Iterators can be traversed either with a `for` loop or with `into_iter()`. [2](https://jyn.dev/the-core-of-rust/#fn-2) `for` is eager and `into_iter` is lazy. `iter` has different ownership semantics than `into_iter`.
...
- `recommended_watcher` only accepts functions that are `Send + 'static`. Small changes to this program, such as passing the current path into the closure, will give a compile error related to ownership. Fixing it requires learning the `move` keyword, knowing that closures borrow their arguments by default, and the meaning of `'static`.
  ...
  - If you are using `Rc<RefCell<String>>`, which is often recommended for beginners[3](https://jyn.dev/the-core-of-rust/#fn-3), your program will need to be rewritten from scratch (either to use Arc/Mutex or to use exterior mutability). For example, if you wanted to print changes from the main thread instead of worker threads to avoid interleaving output, you couldn't simply push to the end of an `all_changes` collection, you would have to use `Arc<Mutex<Vec<Path>>>` in order to communicate between threads.
...
## Rust's core is interwoven on purpose [Anchor link for: rust-s-core-is-interwoven-on-purpose](https://jyn.dev/the-core-of-rust/\#rust-s-core-is-interwoven-on-purpose)
...
## A smaller Rust [Anchor link for: a-smaller-rust](https://jyn.dev/the-core-of-rust/\#a-smaller-rust)
