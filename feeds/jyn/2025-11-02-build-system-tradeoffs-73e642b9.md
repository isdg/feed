...
feed: jyn
guid: https://jyn.dev/build-system-tradeoffs/
---

# build system tradeoffs

This post is part 1/4 of [a series about build systems](https://jyn.dev/four-posts-about-build-systems/). The next post is [negative build dependencies](https://jyn.dev/negative-build-dependencies/).
...
## build concerns [Anchor link for: build-concerns](https://jyn.dev/build-system-tradeoffs/\#build-concerns)

What makes a project’s build complicated?

### running generated binaries [Anchor link for: running-generated-binaries](https://jyn.dev/build-system-tradeoffs/\#running-generated-binaries)
...
### correct dependency tracking [Anchor link for: correct-dependency-tracking](https://jyn.dev/build-system-tradeoffs/\#correct-dependency-tracking)

I actually forgot this one in the first draft because Cargo solves this so well in the common case [1](https://jyn.dev/build-system-tradeoffs/#fn-14). In many hand-written builds ( *cough* `make` *cough*), specifying dependencies by hand is very broken, `-j` for parallelism simply doesn't work, and running `make clean` on errors is common. Needless to say, this is a bad experience.

### cross-compiling [Anchor link for: cross-compiling](https://jyn.dev/build-system-tradeoffs/\#cross-compiling)
...
### toolchains [Anchor link for: toolchains](https://jyn.dev/build-system-tradeoffs/\#toolchains)
...
- A standard library for that target. This is very language-specific, but at a minimum requires a working C toolchain [5](https://jyn.dev/build-system-tradeoffs/#fn-4).
...
### environments [Anchor link for: environments](https://jyn.dev/build-system-tradeoffs/\#environments)
...
### reproducible builds [Anchor link for: reproducible-builds](https://jyn.dev/build-system-tradeoffs/\#reproducible-builds)
...
## tradeoffs [Anchor link for: tradeoffs](https://jyn.dev/build-system-tradeoffs/\#tradeoffs)

At this point, we have enough information to start talking about the space of tradeoffs for a build system.

### configuration language [Anchor link for: configuration-language](https://jyn.dev/build-system-tradeoffs/\#configuration-language)
...
The most common unforced error I see build systems making is forcing the build configuration to be written in a custom language[6](https://jyn.dev/build-system-tradeoffs/#fn-6). There are basically two reasons they do this:
...
- [Starlark](https://starlark-lang.org/) [7](https://jyn.dev/build-system-tradeoffs/#fn-7)
...
### reflection [Anchor link for: reflection](https://jyn.dev/build-system-tradeoffs/\#reflection)
...
Ninjafiles give you the absolute bare minimum necessary to express your build dependencies: You get "rules", which explain *how* to build an output; "build edges", which state *when* to build; and "variables", which say *what* to build[9](https://jyn.dev/build-system-tradeoffs/#fn-12). That's basically it. There's some subtleties about "depfiles" which can be used to dynamically add build edges while running the build rule.
...
### dependency tracking [Anchor link for: dependency-tracking](https://jyn.dev/build-system-tradeoffs/\#dependency-tracking)
...
#### "not my problem, lol" [Anchor link for: not-my-problem-lol](https://jyn.dev/build-system-tradeoffs/\#not-my-problem-lol)
...
##### compiler support [Anchor link for: compiler-support](https://jyn.dev/build-system-tradeoffs/\#compiler-support)
...
#### ephemeral state [Anchor link for: ephemeral-state](https://jyn.dev/build-system-tradeoffs/\#ephemeral-state)
...
#### hermetic builds [Anchor link for: hermetic-builds](https://jyn.dev/build-system-tradeoffs/\#hermetic-builds)
...
The main downside is that you have to actually specify all those dependencies (if you don't, you get a hard error instead of an unsound build graph, which is the main difference between hermetic systems and "not my problem"). Bazel and Buck2 give you starlark, so you have a ~real[11](https://jyn.dev/build-system-tradeoffs/#fn-10) language in which to do it, but it's still a ton of work. Both have an enormous "prelude" module that just defines where you get a compiler toolchain from[12](https://jyn.dev/build-system-tradeoffs/#fn-13).
...
## future work [Anchor link for: future-work](https://jyn.dev/build-system-tradeoffs/\#future-work)
...
## takeaways [Anchor link for: takeaways](https://jyn.dev/build-system-tradeoffs/\#takeaways)
...
## bibliography [Anchor link for: bibliography](https://jyn.dev/build-system-tradeoffs/\#bibliography)
...
1. the uncommon case mostly looks like [incremental bugs in rustc itself](https://blog.rust-lang.org/2021/05/10/Rust-1.52.1/), or issues around rerunning build scripts. [↩](https://jyn.dev/build-system-tradeoffs/#fr-14-1)

2. see [this stackexchange post](https://langdev.stackexchange.com/questions/153/what-are-the-advantages-of-requiring-forward-declaration-of-methods-fields-like/) for more discussion about the tradeoffs between forward declarations and requiring full access to the source. [↩](https://jyn.dev/build-system-tradeoffs/#fr-1-1)

3. even [Rust depends on crt1.o when linking](https://github.com/rust-lang/rust/issues/11937)! [↩](https://jyn.dev/build-system-tradeoffs/#fr-2-1)

4. early binding is called "static linking". [↩](https://jyn.dev/build-system-tradeoffs/#fr-3-1)

5. actually, Zig solved this in [the funniest way possible](https://andrewkelley.me/post/zig-cc-powerful-drop-in-replacement-gcc-clang.html), by bundling a C toolchain with their Zig compiler. This is a legitimately quite impressive feat. If there's any Zig contributors reading—props to you, you did a great job. [↩](https://jyn.dev/build-system-tradeoffs/#fr-4-1)

6. almost every build system does this, so I don't even feel compelled to name names. [↩](https://jyn.dev/build-system-tradeoffs/#fr-6-1)

7. Starlark is *not* tied to hermetic build systems. The fact that the only common uses of it are in hermetic build systems is unfortunate. [↩](https://jyn.dev/build-system-tradeoffs/#fr-7-1)

8. H.T. [Julia Evans](https://jvns.ca/blog/2020/10/26/ninja--a-simple-way-to-do-builds/) [↩](https://jyn.dev/build-system-tradeoffs/#fr-8-1)

9. actually variables are more general than this, but for $in and $out this is true. [↩](https://jyn.dev/build-system-tradeoffs/#fr-12-1)

10. another example is "rebuilding build.ninja when the build graph changes". it's more common than you think because the language is so limited that it's easier to rerun the configure script than to try and fit the dependency info into the graph. [↩](https://jyn.dev/build-system-tradeoffs/#fr-9-1)

11. not actually turing-complete [↩](https://jyn.dev/build-system-tradeoffs/#fr-10-1)

12. I have been informed that the open-source version of Bazel is not actually hermetic-by-default inside of its prelude, and just uses system libraries. This is quite unfortunate; with this method of using Bazel you are getting a lot of the downsides and little of the upside. Most people I know using it are doing so in the hermetic mode. [↩](https://jyn.dev/build-system-tradeoffs/#fr-13-1)

13. there's something *called* `docker compose`, but it composes containers, not images. [↩](https://jyn.dev/build-system-tradeoffs/#fr-11-1)
