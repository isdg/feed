...
feed: jyn
guid: https://jyn.dev/i-want-a-better-action-graph-serialization/
---

# I want a better action graph serialization

This post is part 3/4 of [a series about build systems](https://jyn.dev/four-posts-about-build-systems/). The next post and last post is [I want a better build executor](https://jyn.dev/i-want-a-better-build-executor/).
...
## what does "action graph" mean? [Anchor link for: what-does-action-graph-mean](https://jyn.dev/i-want-a-better-action-graph-serialization/\#what-does-action-graph-mean)
...
This post explores that design space, with the goal of sketching a format that improves on the current state while also enabling incremental adoption. When I say "design space", I mean a serialization format where files are machine-generated by a configure step, and have few enough and restricted enough features that it's possible to make a fast [build executor](https://jyn.dev/i-want-a-better-build-executor/).

### who uses an action graph? [Anchor link for: who-uses-an-action-graph](https://jyn.dev/i-want-a-better-action-graph-serialization/\#who-uses-an-action-graph)
...
- You don’t have a remote build cache. When you have a remote cache, the rules for *loading* that cache can be rather complicated because they involve network queries [1](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-10). When you have a local cache, loading it doesn’t require special support because it’s just opening a file.
...
### design goals [Anchor link for: design-goals](https://jyn.dev/i-want-a-better-action-graph-serialization/\#design-goals)
...
## existing serializations [Anchor link for: existing-serializations](https://jyn.dev/i-want-a-better-action-graph-serialization/\#existing-serializations)

The first one we'll look at, because it's the default for CMake, is `make` and Makefiles. Make is truly in the Unix spirit: easy to implement[2](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-2), very hard to use correctly. Make is [ambiguous](https://medium.com/@adi.ashour/stuff-i-learned-about-makefiles-part-2-66d87109b19b#:~:text=beware%20of%20ambiguity), [complicated](https://medium.com/@adi.ashour/stuff-i-learned-about-makefiles-part-2-66d87109b19b#:~:text==%20vs%20:=), and makes it very easy to [implicitly do a bunch of file system lookups](https://medium.com/@adi.ashour/stuff-i-learned-about-makefiles-part-1-9b85986d7c59#:~:text=pattern%20rules). It supports running shell commands at the top-level, which makes even loading the graph very expensive. It does do pretty well on minimizing reconfigurations, since the language is quite flexible.
...
## dependency edge issues with ninja [Anchor link for: dependency-edge-issues-with-ninja](https://jyn.dev/i-want-a-better-action-graph-serialization/\#dependency-edge-issues-with-ninja)

### negative dependencies [Anchor link for: negative-dependencies](https://jyn.dev/i-want-a-better-action-graph-serialization/\#negative-dependencies)

See [my previous post](https://jyn.dev/negative-build-dependencies/) about negative dependencies. The short version is that build files need to specify not just the files they expect to exist, but also the files they expect *not* to exist. There's no way to express this in a ninja file, short of reconfiguring every time a directory that might contain a negative dependency is modified, which itself has a lot of downsides.

### renamed files [Anchor link for: renamed-files](https://jyn.dev/i-want-a-better-action-graph-serialization/\#renamed-files)

Say that you have a C project with just a `main.c`. You rename it to `project.c` and ninja gives you an error that main.c no longer exists. Annoyed of editing ninja files by hand, you decide to write a generator[3](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-4) :
...
### optional file dependencies [Anchor link for: optional-file-dependencies](https://jyn.dev/i-want-a-better-action-graph-serialization/\#optional-file-dependencies)
...
### environment variables [Anchor link for: environment-variables](https://jyn.dev/i-want-a-better-action-graph-serialization/\#environment-variables)
...
## designing a new action graph [Anchor link for: designing-a-new-action-graph](https://jyn.dev/i-want-a-better-action-graph-serialization/\#designing-a-new-action-graph)
...
6. "all the features of ninja" (depfiles, [monadic builds](https://www.microsoft.com/en-us/research/uploads/prod/2018/03/build-systems-final.pdf) through `dyndeps` [4](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-14), a `generator` statement, order-only dependencies)

Ideally, it would even be possible to mechanically translate existing .ninja files to this new format.

### a first try [Anchor link for: a-first-try](https://jyn.dev/i-want-a-better-action-graph-serialization/\#a-first-try)

This sketches out a new format that could improve over Ninja files. It could look something like this:

- A very very small clojure subset (just `def`, `let`, [EDN](https://github.com/edn-format/edn), and function calls) for the text itself, no need to make loading the graph harder than necessary [5](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-6). If people really want an equivalent of `include` or `subninja` I suppose this could learn support for `ns` and `require`, but it would have much simpler rules than Clojure's classpath. It would not have support for looping constructs, nor most of clojure's standard library.
...
- Runtime functions that determine whether the configure step needs to be re-run based on file watch events[6](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-7). Whether there is actually a file watcher or the build system just calculates a diff on its next invocation is an implementation detail; ideally, one that's easy to slot in and out.
...
- Command spawning is specified as an array [7](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-12). No more dependency on shell quoting rules. If people want shell scripts they can put that in their configure script.
...
- We naturally distinguish between “phony targets” and files because the former are `group` s and the latter are `action` s. No more accidentally failing to build if an `all` file is created. [8](https://jyn.dev/i-want-a-better-action-graph-serialization/#fn-13)
...
### "did you just reinvent [starlark](https://starlark-lang.org/)?" [Anchor link for: did-you-just-reinvent-starlark](https://jyn.dev/i-want-a-better-action-graph-serialization/\#did-you-just-reinvent-starlark)
...
### runners [Anchor link for: runners](https://jyn.dev/i-want-a-better-action-graph-serialization/\#runners)
...
### "are you just piling a bunch of features on top of ninja?" [Anchor link for: are-you-just-piling-a-bunch-of-features-on-top-of-ninja](https://jyn.dev/i-want-a-better-action-graph-serialization/\#are-you-just-piling-a-bunch-of-features-on-top-of-ninja)
...
## next steps [Anchor link for: next-steps](https://jyn.dev/i-want-a-better-action-graph-serialization/\#next-steps)
...
1. see e.g. [this description](https://github.com/facebook/buck2/issues/976#issuecomment-3007201092) of how it works in buck2 [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-10-1)

2. at least a basic version—although several of the features of GNU Make get rather complicated. [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-2-1)

3. `ninja_syntax.py` is [https://github.com/ninja-build/ninja/blob/231db65ccf5427b16ff85b3a390a663f3c8a479f/misc/ninja\_syntax.py](https://github.com/ninja-build/ninja/blob/231db65ccf5427b16ff85b3a390a663f3c8a479f/misc/ninja_syntax.py). [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-4-1)

4. technically these aren't true monadic builds because they're constrained a lot more than e.g. Shake rules, they can't fabricate new rules from whole cloth. but they still allow you to add more outputs to the graph at runtime. [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-14-1)

5. This goes all the way around [the configuration complexity clock](https://mikehadlow.blogspot.com/2012/05/configuration-complexity-clock.html) and skips the "DSL" phase to simply give you a real language. [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-6-1)

6. This is totally based and not at all a terrible idea. [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-7-1)

7. This has a whole bunch of problems on Windows, where arguments are passed as a single string instead of an array, and each command has to reimplement its own parsing. But it will work “most” of the time, and at least avoids having to deal with Powershell or CMD quoting. [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-12-1)

8. To make it possible to distinguish the two on the command line, `:all` could unambiguously refer to the group, like in Bazel. [↩](https://jyn.dev/i-want-a-better-action-graph-serialization/#fr-13-1)
