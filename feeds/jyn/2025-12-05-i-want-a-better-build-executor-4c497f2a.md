...
feed: jyn
guid: https://jyn.dev/i-want-a-better-build-executor/
---

# I want a better build executor

This post is part 4/4 of [a series about build systems](https://jyn.dev/four-posts-about-build-systems/).
...
In [my last post](https://jyn.dev/i-want-a-better-action-graph-serialization/), I describe an improved build graph serialization. In this post, I describe the build executor that reads those files.

## what is a build executor? [Anchor link for: what-is-a-build-executor](https://jyn.dev/i-want-a-better-build-executor/\#what-is-a-build-executor)

Generally, there are three stages to a build:

1. Resolving and downloading dependencies. The tool that does this is called a [**package manager**](https://nesbitt.io/2025/12/02/what-is-a-package-manager.html). Common examples are `npm`, `pip`, [Conan](https://docs.conan.io/2/index.html) [1](https://jyn.dev/i-want-a-better-build-executor/#fn-1), and the [`cargo` resolver](https://doc.rust-lang.org/cargo/reference/resolver.html).
...
## change detection [Anchor link for: change-detection](https://jyn.dev/i-want-a-better-build-executor/\#change-detection)
...
## querying [Anchor link for: querying](https://jyn.dev/i-want-a-better-build-executor/\#querying)

I wrote earlier about [querying the build graph](https://jyn.dev/build-system-tradeoffs/#reflection). There are two kinds of things you can query: The configuration graph (what bazel calls the [target graph](https://bazel.build/query/language)), which shows dependencies between "human meaningful" packages; and the [action graph](https://bazel.build/query/aquery), which shows dependencies between files.
...
Note that “querying the graph” is not a binary yes/no. Ninja's query language is much more restricted than Bazel's. Compare Ninja's syntax for querying “the command line for all C++ files used to build the target `//:hello_world`” [2](https://jyn.dev/i-want-a-better-build-executor/#fn-3):
...
## tracing [Anchor link for: tracing](https://jyn.dev/i-want-a-better-build-executor/\#tracing)

In [my previous post](https://jyn.dev/build-system-tradeoffs/), I talked about two main uses for a tracing build system: first, to automatically add dependency edges for you; and second, to verify at runtime that no dependency edges are missing. This especially shines when the action graph has a way to express negative dependencies, because the tracing system *sees* every attempted file access and can add them to the graph automatically.
...
### environment variables [Anchor link for: environment-variables](https://jyn.dev/i-want-a-better-build-executor/\#environment-variables)
...
## `ronin`: a ninja successor [Anchor link for: ronin-a-ninja-successor](https://jyn.dev/i-want-a-better-build-executor/\#ronin-a-ninja-successor)

Here I describe more concretely the tool I want to build, which I’ve named `ronin`. It would read the [constrained clojure action graph serialization format](https://jyn.dev/i-want-a-better-action-graph-serialization/#designing-a-new-action-graph) (Magma) that I describe in the previous post; perhaps with a way to automatically convert Ninja files to Magma.

### interface [Anchor link for: interface](https://jyn.dev/i-want-a-better-build-executor/\#interface)
...
### architecture [Anchor link for: architecture](https://jyn.dev/i-want-a-better-build-executor/\#architecture)
...
Tracing would be built on top of a FUSE file system that tracked file access. [3](https://jyn.dev/i-want-a-better-build-executor/#fn-2)
...
## did you just reinvent buck2? [Anchor link for: did-you-just-reinvent-buck2](https://jyn.dev/i-want-a-better-build-executor/\#did-you-just-reinvent-buck2)
...
## summary [Anchor link for: summary](https://jyn.dev/i-want-a-better-build-executor/\#summary)
...
I don’t know yet if I will actually build this tool, that seems like a lot of work [5](https://jyn.dev/i-want-a-better-build-executor/#fn-4) 😄 but it’s something I would like to exist in the world.

1. In many ways Conan [profiles](https://docs.conan.io/2/tutorial/consuming_packages/build_simple_cmake_project.html) are analogous to ninja files: profiles are the interface between Conan and CMake in the same way that ninja files are the interface between CMake and Ninja. Conan is the only tool I'm aware of where the split between the package manager and the configure step is explicit. [↩](https://jyn.dev/i-want-a-better-build-executor/#fr-1-1)

2. This is not an apple to apples comparison; ideally we would name the target by the output file, not by its alias. Unfortunately output names are unpredictable and quite long in Bazel. [↩](https://jyn.dev/i-want-a-better-build-executor/#fr-3-1)

3. macOS does not have native support for FUSE. [MacFuse](https://macfuse.github.io/) exists but does not [support getting the PID](https://github.com/macfuse/macfuse/wiki/FUSE-Backends#limitations) of the calling process. A possible workaround would be to start a new FUSE server for each spawned process group. FUSE on Windows is possible through [winfsp](https://github.com/billziss-gh/winfsp). [↩](https://jyn.dev/i-want-a-better-build-executor/#fr-2-1)

4. An earlier version of this post read "Buck2 only supports non-hermetic builds for [system toolchains](https://buck2.build/docs/concepts/toolchain/), not anything else", which is not correct. [↩](https://jyn.dev/i-want-a-better-build-executor/#fr-buck-correction-1)

5. what if i simply took buck2 and hacked it to bits,,, [↩](https://jyn.dev/i-want-a-better-build-executor/#fr-4-1)
