...
feed: jyn
guid: https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/
---
...
## the setup [Anchor link for: the-setup](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#the-setup)
...
## the plan [Anchor link for: the-plan](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#the-plan)

*In which our intrepid heroes chart the challenges to come.*

### initramfs [Anchor link for: initramfs](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#initramfs)

Oh, right. If you weren’t aware, early boot in a Linux operating system[1](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fn-3) is just running a full second operating system that happens to be very small, lol. That’s loaded from a compressed archive file in /boot[2](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fn-4) and run from memory, with no access to persistent storage. This OS running from memory is called **initramfs** (initial RAM filesystem).

So when you see a screen like this: ![](https://jyn.dev/assets/Pasted%20image%2020260121044155.png) That’s actually a whole-ass OS, with an `init` PID and service management and everything. This is how, for example, `systemd-analyze` can show you stats about early boot — there’s another copy of systemd running in initramfs, and it passes its state off to the one in the main OS.

Well. That implies we can install things on it ^^.

### constraints [Anchor link for: constraints](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#constraints)
...
### tailscale ACLs [Anchor link for: tailscale-acls](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#tailscale-acls)
...
`selene` here is my desktop, and `selene-initrd` is its initramfs. [3](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fn-1)

### systemd before boot [Anchor link for: systemd-before-boot](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#systemd-before-boot)
...
## the heist [Anchor link for: the-heist](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#the-heist)
...
- In [the tailscale web console](https://login.tailscale.com/admin/machines), mark your new device with `tag:initrd`, and disable key expiry. It should look something like this:
  ...
  ![](https://jyn.dev/assets/Pasted%20image%2020260121213235.png)
...
- Tell systemd to wait forever for a decryption password. I use `systemd-boot`, so I edited `/boot/loader/entries/linux-cachyos`. Under `options`, I extended the existing `rootflags=subvol=/@` to `rootflags=subvol=/@,x-systemd.device-timeout=0`. [4](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fn-2)
...
![](https://jyn.dev/assets/Screenshot_20260121_222102.png)

## the getaway [Anchor link for: the-getaway](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/\#the-getaway)
...
1. and I believe in Windows, although I’m less sure about that [↩](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fr-3-1)

2. sometimes /boot/EFI [↩](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fr-4-1)

3. Here “initrd” stands for “initramdisk”, which is another word for our initramfs system. [↩](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fr-1-1)

4. See [the `sd-dropbear`
   docs](https://github.com/wolegis/mkinitcpio-systemd-extras/wiki/Dropbear-SSH-server#:~:text=systemd%2Edevice%2Dtimeout) for more information about this. [↩](https://jyn.dev/remotely-unlocking-an-encrypted-hard-disk/#fr-2-1)
