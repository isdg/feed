...
feed: rust-lang
guid: https://blog.rust-lang.org/2026/07/09/Rust-1.97.0/
---
...
When Rust is compiled into object files and binaries, each item (functions, statics, etc) must have a globally unique "symbol" identifying it. To avoid conflicts when linking together different Rust programs, Rust mangles the original name of items to include additional context such as the module path, defining crate, generics, and more. Historically, this mangling was based on the [Itanium ABI](https://refspecs.linuxbase.org/cxxabi-1.86.html#mangling), also (sometimes) used by C++.
...
To silence the warning in the mean time, you can configure the lint level to allow. This can be done through `Cargo.toml` by adding a [lints section](https://doc.rust-lang.org/nightly/cargo/reference/manifest.html#the-lints-section) like this:
